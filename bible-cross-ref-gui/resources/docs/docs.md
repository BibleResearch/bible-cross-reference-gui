...
The `home-routes` handler in the `bible-cross-ref-gui.routes.home` namespace
...
The `home-page` function will in turn call the `bible-cross-ref-gui.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `bible-cross-ref-gui.handler` namespace:
...
Request middleware functions are located under the `bible-cross-ref-gui.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `bible-cross-ref-gui.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `bible-cross-ref-gui.db.core` in some other namespace.
